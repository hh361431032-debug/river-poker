...
function startHand(room){
  ...
  r.players=r.players.filter(p=>p.chips>0&&!p.kicked);
  if(r.players.length<2){r.stage="waiting";r.status="waiting";return r;}
  r.players.forEach(p=>Object.assign(p,{cards:[],folded:false,allIn:false,bet:0,totalContributed:0,hasActed:false,inHand:true,waitingForNext:false,kicked:false}));
  r.deck=shuffle(freshDeck());r.community=[];r.pot=0;r.currentBet=0;r.minRaise=BIG_BLIND;r.log=[];r.handNumber=(r.handNumber||0)+1;r.status="playing";r.stage="preflop";r.turnStartedAt=Date.now();
  ...
  blind(sb,SMALL_BLIND);blind(bb,BIG_BLIND);r.currentBet=BIG_BLIND;r.turnIndex=first;r.turnStartedAt=Date.now();r.log.push(`第 ${r.handNumber} 局开始，庄家：${r.players[r.dealerIndex].name}`);return r;
}
  ...
  return <div className="page lobby"><header><div className="brand small-brand"><span>♠</span><h1>河畔牌局</h1></div><div className="user-area">{isAdmin&&<span className="admin-badge"><Shield size={14}/>管理员</span>}{username}<button className="icon" onClick={onLogout}><LogOut size={16}/></button></div></header>
    <main className="lobby-grid">
      ...
      <section className="panel wide"><div className="panel-head"><h2>公开房间</h2><button className="icon" onClick={refresh}><RefreshCw size={15}/></button></div>
        {!rooms.length?<p className="muted">暂无房间，创建一个开始游戏吧。</p>:rooms.map(r=><div className="room-row" key={r.code}><div><b>{r.name}</b><small>房主 {r.hostName} · <Users size={12}/> {r.playerCount}/{MAX_PLAYERS} · {r.status==="playing"?"进行中":"等待中"} · {r.code}</small></div><div className="room-actions"><button className="small-button" onClick={()=>joinRoom(r.code)}>加入</button>{isAdmin&&<button className="danger icon" title="删除房间" onClick={()=>deleteRoom(r.code)}><Trash2 size={15}/></button>}</div></div>)}
      </section>
    </main></div>;
}

function WaitingRoom({room,username,onStart,onLeave,onKick}){
...
function RoomController({code,username,onLeaveLobby}){
  ...
  const save=async(r)=>{setRoom(r);await storage.set(`poker:room:${code}`,JSON.stringify(r));};
